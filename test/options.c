...
static void count_options(const option_t *option, void *_count) {
	(*(int *) _count)++;
}
...
static void test_for_each() {
	...
	options_init(&options);
	option_t option1;
	option_init(&option1, 'a', "first", "First option.", nothing);
	options_add(&options, option1);
	option_t option2;
	option_init(&option2, 'b', "second", "Second option.", nothing);
	options_add(&options, option2);

	int count = 0;
	options_for_each(&options, count_options, &count);
	assert(count == 2);

	option_destroy(&option1);
	option_destroy(&option2);
	options_destroy(&options);
}
